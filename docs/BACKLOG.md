...
### 1. History management / archive UX

Почему важно:
- list-first и recovery UX уже работают, но рабочие экраны перегружает полная лента запусков
- автоматическая retention по дням уже есть, а управляемого archive/cleanup слоя пока нет

Что должно быть:
- отдельный archive-экран для полной истории запусков
- на `/`, `/jobs`, `/runs` оставлять только active, interrupted, repeated-failure и короткий recent slice
- pagination или page-based navigation для длинной истории
- ручное удаление отдельных запусков и массовая cleanup-очистка старых локальных записей и логов
- cleanup как дополнение к retention, а не как ее замена

### 2. Backup / restore конфигурации приложения без секретов
...
### 3. Связка диагностики профилей с `Ops / Runbook`
...
- уже есть явные переходы из диагностики профиля в релевантные сценарии `/ops`; следующий шаг — расширять покрытие и делать сценарии еще конкретнее
- более плотная связь между `последняя проверка`, `что проверить` и `что сделать дальше`

## P1

### 4. Операционный UX: нотификации, `launchd`, перенос на другой `Mac`
...
### 5. Guided setup для `rclone remote` через UI
...
### 6. Более богатый operational runbook
...
### 7. Дополнительный content polish UI
...
- адресная докрутка helper-блоков, counters и empty states без возврата к широкому редизайну

### 8. Более явная диагностика mount path для Synology / SMB
