...
- полноценного re-attach к уже живому `rclone` процессу после restart пока нет
...
- recovery UX для `interrupted` запусков в `/runs` и `/runs/{id}`
...
- отдельный guided recovery flow, а не только пометка `interrupted`
...
- автоматического продолжения уже стартовавшего `rclone` процесса после рестарта пока нет
...
1. Добавить dry-run preview перед первым запуском.
2. Доработать recovery-flow: явные действия для `interrupted` задач и повтор с дельты из UI.
3. Решить, нужен ли re-attach к живому `rclone` процессу после рестарта, или достаточно recovery через повторный `copy`.
4. Улучшить диагностику профилей и cloud setup.
5. После этого доработать операционный слой: launchd, retention, backup/restore конфигурации.
