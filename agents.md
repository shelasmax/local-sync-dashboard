...
- релиз `v2.1.1` уже зафиксировал явную диагностику timeout для long-running run, показ реально перенесённого объёма в run detail и минимальную operational-настройку лимита одного запуска в `/ops`; следующий шаг в этой зоне — не городить новый timeout UX, а развивать profile defaults и backup/restore.
...
- при анализе repeated retries для одного и того же маршрута помнить: truth source для «что реально осталось сейчас» — это live runtime snapshot и фактическая дельта `rclone`, а не summary предыдущего run в изоляции.
