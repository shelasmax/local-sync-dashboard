...
1. Archive UX для запусков: отдельная история, pagination, ручная cleanup-очистка локального состояния и более короткие operational surfaces
2. Экспорт и backup/restore конфигурации приложения без секретов
3. Связка диагностики профилей с `Ops / Runbook` и более глубокие action-oriented подсказки
4. Операционный UX: нотификации, `launchd`-сценарии и перенос на другой `Mac`
5. Guided setup для `rclone remote`
...
- автоматическая retention run history по дням уже есть; следующий шаг по lifecycle — отдельный архив запусков, pagination и ручная cleanup-очистка без затрагивания данных в source/target.
