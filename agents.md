...
- базовая ручная перепроверка профилей, hints, сохранение последнего результата проверки, richer hints по типам ошибок и action-oriented CTA уже есть; прямые переходы из диагностики в сценарии `/ops` тоже уже есть. Следующий шаг — углублять покрытие сценариев и делать runbook еще более action-oriented.
